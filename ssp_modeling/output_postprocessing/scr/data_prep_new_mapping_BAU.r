...
data_new$strategy <- gsub("NDC","BAU + NDC",data_new$strategy)
data_new$strategy <- gsub("NZ","BAU + Net Zero",data_new$strategy)

#write file 
dir.out <- paste0("ssp_modeling/Tableau/data/")
file.name <- "emissions_bau.csv"

write.csv(data_new,paste0(dir.out,file.name),row.names=FALSE)

print('Finish: data_prep_new_mapping_2025_04_28_BAU process')